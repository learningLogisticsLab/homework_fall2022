...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for i in range(self.num_target_updates):  # performs best when num_target_updates is must smaller than num_grad_steps_per_target_update
            vs_prime = self.critic_network.forward(next_ob_no).detach().squeeze() * (1 - terminal_n)
            target = reward_n + self.gamma * vs_prime
            for j in range(self.num_grad_steps_per_target_update):
                self.optimizer.zero_grad()
                values = self.critic_network.forward(ob_no).squeeze()

                assert target.shape == values.shape 
                
                loss = self.loss(target, values)
                loss.backward()
                self.optimizer.step()
